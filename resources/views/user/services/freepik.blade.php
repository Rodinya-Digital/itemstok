...
@section('topbuttons')
    ...
    background: radial-gradient(circle, #62a3f8, #c0dafd);
">
        <div>
            <svg width="160" height="44" fill="#fff" class="_121cna13 $block" aria-labelledby="logo-:R19bqbm:"
                 viewBox="0 0 712 105" xmlns="http://www.w3.org/2000/svg">
       ...
       {{-- <div>
            <h3 class="p-0 m-0">Freepik Premium</h3>
        </div>--}}
...
@section('content')
    ...
    @if($data)
        {{-- <div class="alert alert-danger fw-bold fs-4">
             Şu anda Freepik sistemlerini güncelliyor, bu nedenle indirmelerde zaman zaman hatalar yaşayabilirsiniz. Bu bizden kaynaklanmamaktadır. Güncellemeler tamamlanana kadar kesintiler olabilir.
             <hr>
             Currently Freepik is updating their system, so you may experience occasional errors with downloads. This is not caused by us. There may be interruptions until the updates are complete.
         </div>--}}
        ...
        <div class="card p-4" style="background: radial-gradient(circle, #62a3f8, #c0dafd);">
          {{--  <div class="alert alert-danger fw-bold">
                @if(auth()->user()->locale!='tr')
                    A workaround has been implemented for Freepik video downloads.<br>
                    The larger the video size, the longer it will take to download.<br>
                    1GB corresponds to approximately 1 minute.<br>
                    2GB corresponds to approximately 2 minutes.<br>
                    If you get an error, please read the error details carefully!<br>
                @else
                    Freepik video indirmeleri için geçici bir çözüm uygulandı.<br>
                    Video boyutu ne kadar büyükse, indirilmesi o kadar uzun sürecektir.<br>
                    1GB yaklaşık 1 dakikaya karşılık gelir.<br>
                    2GB yaklaşık 2 dakikaya karşılık gelir.<br>
                    Bir hata alırsanız, lütfen hata ayrıntılarını dikkatlice okuyun!<br>
                @endif
            </div>--}}


            <div class="d-flex align-items-center">
                <input type="text" class="form-control rounded-end-0" style="color:#182453;" id="freepik_url"
                       placeholder="{{__("Please enter the content URL.")}}">
                <button class="btn btn-light btn-sm rounded-start-0" style="width: 300px!important;height: 43px!important;"
                        id="freepik_down">{{__("Create Link")}}</button>
            </div>
                ...
                <script>
                    ...
                    $('#freepik_down').click(() => {
                        ...
                        $.post('<?= route('api.freepik') ?>', {
                            ...
                            if (response.types) {
                                swal.fire({
                                    title: '{{__("Please select in which quality you would like to download this content.")}}',
                    ...
                    function videoTypeButton(datas) {
                        ...
                        datas.forEach((v, i) => {
                            if (v.active) {
                                datax += `<button id="videoType" data-id="${v.id}" class="btn btn-warning btn-block mb-2">
<b>${v.quality} / ${v.width} x ${v.height} / ${v.container}</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-download"></i><br>
${v.size} MB / ${v.codec} / ${v.bitRate} Mbps
</button>`
                            }
                        })
                        return datax;
